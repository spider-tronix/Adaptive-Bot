...
class Aragog:
    def __init__(self):
	self.left_revolute_bottom_angle = 0 * 3.14 / 180.0
    	self.left_revolute_top_angle = 20 * 3.14 / 180.0
    	self.right_revolute_bottom_angle = 0 * 3.14 / 180.0
    	self.right_revolute_top_angle = 20 * 3.14 / 180.0
    	self.middle_revolute_left_angle = -30 * 3.14 / 180.0
    ...
    def move_forward(self):
        ...
        self.left_revolute.publish(self.left_revolute_top_angle)
        time.sleep(0.5)
        self.right_revolute.publish(self.right_revolute_bottom_angle)
        time.sleep(0.5)

        self.middle_revolute.publish(self.middle_revolute_right_angle)
        time.sleep(1.5)
        self.left_revolute.publish(self.right_revolute_bottom_angle)
        time.sleep(0.5)
        self.right_revolute.publish(self.right_revolute_top_angle)
        time.sleep(0.5)
